...
import ambientMusic from "@/assets/music/ambientspace.mp3";
...
export const MusicToggleButton = () => {
  ...
  const [play, { pause, sound }] = useSound(ambientMusic, {
    loop: true,
  ...
  return (
    <>
      <motion.div
        ...
        className="bg-audio cursor-pointer rounded-lg px-2"
      >
        <motion.div
        ...
        >
          {heights.map((height, index) => (
            <motion.div
              key={index}
              className="bg-foreground w-[5px] rounded-full"
              initial={{ height: 1 }}
              animate={{
